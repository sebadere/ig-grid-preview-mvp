        ...
        <div className="max-w-6xl mx-auto px-4 py-12 grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <p className="text-[var(--muted)] mt-4">
              Connect your Notion database and see exactly how your Instagram feed will look. 
              Real-time sync with your content calendar, embedded directly in Notion with 
              <kbd className="px-1.5 py-0.5 rounded border">/embed</kbd>.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              {isConnected ? (
                <Link to="/studio" className="px-4 py-2 rounded-xl bg-black text-white">Open Your Studio</Link>
              ) : (
                <>
                  <Link to="/onboarding" className="px-4 py-2 rounded-xl bg-black text-white">Connect Notion</Link>
                  <Link to="/studio" className="px-4 py-2 rounded-xl border border-[var(--notion-border)] bg-[var(--notion-card)]">Try Demo</Link>
                </>
              )}
              <Link to="/widget?embed=1" className="px-4 py-2 rounded-xl border border-[var(--notion-border)] bg-[var(--notion-card)]">View Embed</Link>
            </div>
            <ul className="mt-8 space-y-2 text-sm text-[var(--muted)]">
              <li>• {isConnected ? '✓ Connected to your Notion database' : '• Connect any Notion database with images'}</li>
              <li>• 3-column Instagram grid with custom spacing</li>
              <li>• {isConnected ? 'Real-time sync with your content' : 'Drag & drop reordering'}</li>
              <li>• Shareable embeds with personalized data</li>
            </ul>
          </div>
